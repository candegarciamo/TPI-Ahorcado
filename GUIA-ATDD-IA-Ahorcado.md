...
**interno** es sobre el objeto **`Ahorcado`** (Vitest): lógica pura, sin DOM.        --------------------         Ver tema de vitest
...
│                                                                     │
│   ┌─ LOOP INTERNO (UT con Vitest, sobre el objeto Ahorcado) ──┐     │            --------------------         Ver tema de vitest
...
> **Y la integración continua queda verde.** Pusheás **solo cuando estás en verde**:  ---------- Hay que hacer lo de CI
...
>
> **Cada Historia de Usuario suele mapear a varios ATs.** Una US del Story Map                --- ------ Story Map
...
Corré `npm run at`. **Predecí:** ¿rojo o verde? (Rojo: el input todavía no existe.)  ------- usar pnpm en ves de pnpm
Confirmá. Este rojo es **honesto**: si la app ni siquiera arranca, también falla.
Fijate que los steps se **reutilizan** entre features (el `Dado una partida…` ya lo   -------  no entendi esto
escribiste para el AT1).

### Paso 2 — Enumerá los UT del objeto `Ahorcado` (vos, antes de pedirle a la IA)         -------- HAY QUE HACER ESTO
...
Corré `npm run at`: tiene que estar **verde**.                                             ----- pnpm en ves de npm
...
el **comportamiento** que automatizaste, pero el navegador te muestra lo que el AT        ----- pnpm en ves de npm
...
Setup mínimo:                         ----- pnpm en ves de npm
